...
connector = sqlite3.connect('FACULTYMANAGEMENT.db')
cursor = connector.cursor()
connector.execute(
"CREATE TABLE IF NOT EXISTS FACULTY_MANAGEMENT (FACULTY_ID INTEGER PRIMARY KEY AUTOINCREMENT NOT NULL, NAME TEXT, EMAIL TEXT, PHONE_NO TEXT, GENDER TEXT, DOB TEXT, STREAM TEXT)"
)
...
def display_records():
   tree.delete(*tree.get_children())
   curr = connector.execute('SELECT * FROM FACULTY_MANAGEMENT')
   data = curr.fetchall()
   for records in data:
...
def add_record():
   ...
   if not name or not email or not contact or not gender or not DOB or not stream:
       ...
       try:
           connector.execute(
           'INSERT INTO FACULTY_MANAGEMENT (NAME, EMAIL, PHONE_NO, GENDER, DOB, STREAM) VALUES (?,?,?,?,?,?)', (name, email, contact, gender, DOB, stream)
           )
           connector.commit()
...
def remove_record():
   if not tree.selection():
       ...
       connector.execute('DELETE FROM FACULTY_MANAGEMENT WHERE STUDENT_ID=%d' % selection[0])
       connector.commit()
...
main = Tk()
main.title('Faculty Database Management System')
...
Label(main, text="FACULTY DATABASE MANAGEMENT SYSTEM", font=headlabelfont, bg='SpringGreen').pack(side=TOP, fill=X)
...
Label(right_frame, text='Faculty Records', font=headlabelfont, bg='DarkGreen', fg='LightCyan').pack(side=TOP, fill=X)
tree = ttk.Treeview(right_frame, height=100, selectmode=BROWSE,
                   columns=('Faculty ID', "Name", "Email Address", "Contact Number", "Gender", "Date of Birth", "Stream"))
X_scroller = Scrollbar(tree, orient=HORIZONTAL, command=tree.xview)
Y_scroller = Scrollbar(tree, orient=VERTICAL, command=tree.yview)
X_scroller.pack(side=BOTTOM, fill=X)
Y_scroller.pack(side=RIGHT, fill=Y)
tree.config(yscrollcommand=Y_scroller.set, xscrollcommand=X_scroller.set)
tree.heading('Faculty ID', text='ID', anchor=CENTER)
